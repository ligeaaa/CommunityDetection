...
from app.ui.ui_load import qss_loader
...
def start():
    ...
    logger.add(os.path.join("c:/tmp/", f'community-{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}.log'),
               format=logformat, rotation="500 MB",
               level=logging.DEBUG)
