...
def validate_input(raw_data, truth_table):
    ...
    if not isinstance(raw_data, list) or not all(
            isinstance(edge, list) and len(edge) == 2 and all(isinstance(node, int) for node in edge)
    ...
    if not isinstance(truth_table, list) or not all(
            isinstance(entry, list) and len(entry) == 2 and all(isinstance(value, int) for value in entry)
...
def display_community_overlap_info(truth_table):
    ...
    if is_overlapping:
        ...
        for membership_count, count in sorted(community_counts.items()):
            logger.info(f"Nodes belonging to {membership_count} community/communities: {count}")
    else:
        logger.info("The community structure is non-overlapping, with each node belonging to only one community.")
...
def get_dataset_info(raw_data, truth_table):
    ...
    logger.info(f"Dataset info - Nodes: {num_nodes}, Edges: {num_edges}, Communities: {num_communities}")
    return {
